                        ...
                        <table class="min-w-full bg-white border border-gray-200">
                            <thead class="bg-gray-100 border-b">
                                <tr>
                                    ...
                                    <th class="py-3 px-5 text-left font-medium text-gray-700">Event Created</th>
                                    ...
                                    <tr class="border-b hover:bg-gray-100">
                                        <td class="py-4 px-5">
                                            <div class="font-semibold text-gray-800">{{ $event->title }}</div>
                                                <div class="text-gray-500 text-sm">{{ $event->deptType->name }} . {{ $event->location->versity_name }}</div>

                                            {{-- <div class="text-gray-500 text-sm">{{ $event->deptType->name }} • {{ $event->location }}</div> --}}
                                        </td>
                                        <td class="py-4 px-5">{{ \Carbon\Carbon::parse($event->created_at)->format('d M, Y') }}</td>
                                        <td class="py-4 px-5 text-gray-700">0 Applications</td>
                                        <td class="py-4 px-5">
                                            <span class="px-3 py-1 rounded-full text-sm {{ $event->status == 1 ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800' }}">
                                                {{ $event->status == 1 ? 'Active' : 'Block' }}
                                            </span>
                                        </td>
                                        <td class="py-4 px-5">
                                            <div class="relative">
                                                <a class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" href="#" onclick="deleteEvent({{ $event->id }})"><i class="fa fa-trash mr-2" aria-hidden="true"></i> Delete</a>
...
function deleteEvent(eventId) {
    if (confirm("Are you sure you want to delete?")) {
        $.ajax({
            url : '{{ route("account.deleteEvent") }}',
            type: 'post',
            data: {eventId: eventId},
            dataType: 'json',
            success: function(response) {
                window.location.href='{{ route("account.myEvents") }}';
            }
        });
    } 
}
